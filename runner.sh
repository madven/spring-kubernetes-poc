#!/usr/bin/env bash
usage() { echo "Usage: $0 [g|n|d|e|i]" 1>&2; exit 1; }
...
if [[ $g -eq 1 ]]; then
  ...
  cd greeting-service
  echo "${yellow}--- mvn clean ---${reset}"
  mvn clean
  echo "${yellow}--- mvn dockerbuild ---${reset}"
...
if [[ $d -eq 1 ]]; then
  ...
  mvn install dockerfile:build
  echo "${yellow}--- push ---${reset}"
  docker push selcukc/dashboard
  cd ..
fi

if [[ $e -eq 1 ]]; then
  echo "${cyan}------------------ eureka-server ------------------${reset}"
  cd eureka-server
  echo "${yellow}--- mvn clean ---${reset}"
  mvn clean
  echo "${yellow}--- mvn dockerbuild ---${reset}"
  mvn install dockerfile:build
  echo "${yellow}--- push ---${reset}"
  docker push selcukc/eureka-server
  cd ..
fi
